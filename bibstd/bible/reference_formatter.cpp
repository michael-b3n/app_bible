...
#include "bibstd/bible/reference_formatter_en.hpp"
...
///
///
auto reference_formatter::operator()(const util::language language, const reference_range& range) -> std::string
{
  static constexpr auto format = [](const auto& pretty_names, const reference_range& range) -> std::string
  {
  ...
  switch(language)
  {
  case util::language::german: return format(reference_formatter_de::pretty_names, range);
  case util::language::english: return format(reference_formatter_en::pretty_names, range);
  default: throw util::exception{"unsupported language"}; return std::string{};
  }
}
