...
GtkMenuShell* Tray::Tray::construct(const std::vector<std::shared_ptr<TrayEntry>>& entries, Tray* parent)
{
  ...
  for(const auto& entry : entries)
  {
    ...
    else if(auto* submenu = dynamic_cast<Submenu*>(item); submenu)
    {
      ...
      gtk_menu_item_set_submenu(reinterpret_cast<GtkMenuItem*>(gtkItem), reinterpret_cast<GtkWidget*>(construct(submenu->getEntries(), parent)));
    }
    else if(auto* iconButton = dynamic_cast<ImageButton*>(item); iconButton)
    {
      ...
      if(parent)
      {
        parent->imageWidgets.emplace_back(reinterpret_cast<GtkContainer*>(box), reinterpret_cast<GtkWidget*>(image));
      }
    }
