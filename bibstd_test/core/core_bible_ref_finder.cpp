...
TEST_CASE("reference_parser", "[bible]")
{
  ...
  const auto ref_exodus_2_2 = bible::reference::create_unguarded(bible::book_id::exodus, 2u, 2u);
  const auto ref_exodus_2_3 = bible::reference::create_unguarded(bible::book_id::exodus, 2u, 3u);
  const auto ref_exodus_2_7 = bible::reference::create_unguarded(bible::book_id::exodus, 2u, 7u);
  const auto ref_exodus_4_5 = bible::reference::create_unguarded(bible::book_id::exodus, 4u, 5u);
  const auto ref_exodus_5_6 = bible::reference::create_unguarded(bible::book_id::exodus, 5u, 6u);
  const auto ref_exodus_7_8 = bible::reference::create_unguarded(bible::book_id::exodus, 7u, 8u);
  ...
  CHECK(core.parse(filler_text, 0, util::language::german, versification).ranges.empty());
  {
    const auto reference = std::string{"1.Mose 1:1"};
    const auto result = core.parse(reference, 0, util::language::german, versification).ranges;
    CHECK(result.size() == 1);
